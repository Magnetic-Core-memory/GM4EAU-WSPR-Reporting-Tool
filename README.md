...
The software has been tested on Microsoft Excel 2010, 2013, 2016, and Office365 (at Jul 19 with Excel 2019.) on Windows 7 and Windows 10. Not tested with Windowd 11.
32 Bit and 64 bit Excel  are supported.  
Open Office and similar are not supported (they don’t provide VBA).  Apple MAC is not currently supported.
...
Operating with 1000-2000 lines of WSPR data should not incur any processing delays of more than a few seconds to request data or draw a Map. The timeline may take much longer if a very high number of time slots are requested. The software will run up to 50,000+  lines (for archived data  analysis activities) however the code is currently limited at 3000 lines.
Note that the wsprnet.org database sometimes experiences lock-outs and delays of up to 120 seconds on data requests are common (at Sep 2019.)
Author contact details are available within the About button on the DATA tab within the WATT tool or via email : gm4eau at G Mail dott com.
